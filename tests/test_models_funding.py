...
from credit_portfolio.models import FlatFundingRate
...
def test_flat_funding_rate_produces_expected_first_period_cost():
    facility = revolver_facility()
    schedule = project_facility_cashflows(facility, funding_model=FlatFundingRate(0.02))

    first = schedule.events[0]
    period_fraction = (first.date - START).days / 365.0
    assert first.funding_cost == pytest.approx(0.02 * 600_000.0 * period_fraction)


def test_no_funding_model_reproduces_zero_funding_cost():
    facility = revolver_facility()
    schedule = project_facility_cashflows(facility)
    assert all(e.funding_cost == 0.0 for e in schedule.events)
